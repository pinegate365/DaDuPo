...
from DaDuPo.data.Asap2Database import Asap2Parameter, Asap2Signal, ParameterType
from DaDuPo.device.DeviceManager import DeviceManager
from DaDuPo.icon.icon import Icon
from DaDuPo.widgets.ArrayWidget import ArrayParameterWidget
from DaDuPo.widgets.ChartWidget import ChartWidget
from DaDuPo.data.DataPool import DataPool
from DaDuPo.widgets.MeasurementConfigDialog import MeasurementConfigDialog
from DaDuPo.widgets.ScalarWidget import ScalarParameterWidget, ScalarSignalWidget, ScalarBaseWidget
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def load_project(self,project):
        project_file = str(Path(project).joinpath("project.json"))
        try:
            with open(project_file) as f:
                config = json.load(f)
                self.device_manager.load_devices(project,config['devices'])
                setting_file  = Path(project) / '{}.mcs'.format(config['name'].lower())
                self.load_gui(str(setting_file))
        except:
            pass
    ...
    def createActions(self):
        ...
        self.startMeasurementAct = QAction(QtGui.QIcon(str(Path(__file__).parent.joinpath('icon/play.svg'))), "&Start Measurement", self,
                                           triggered=self.start_measurement)
        self.startMeasurementAct.setEnabled(False)
        self.stopMeasurementAct = QAction(QtGui.QIcon(str(Path(__file__).parent.joinpath('icon/stop.svg'))), "&Stop Measurement", self,
    ...
    def createToolBars(self):
        ...
        self.device_bar.setObjectName("Device")
